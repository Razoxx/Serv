package l2p.commons.collections;
 ...
 * Не потоко-безопасная версия. Порядок элементов при удалении не сохраняется. В
 * качестве аргумента при добавлении элементов может быть <tt>null</tt>.
 * <p>
 * В качестве параметра при создании обьекта <tt>LazyArrayList</tt> задается
 * начальный размер массива элементов <tt>initialCapacity</tt>.
 * </p>
 * При добавлении элементов, в случае переполнения массива, размер увеличивается
 * на <tt>capacity * 1.5</tt>
 * При удалении элемента, массив не сокращается, вместо этого последний элемент
 * массива становится на место удаленного. Массив элементов обнуляется, если
 * количество элементов в нем равно <tt>0</tt>. При получении элементов с
 * помощью метода {@link List#get(int) List.get(int)}, как и при итерации, может
 * вернуть null, в случае, если индекс выходит за пределы массива.
 * <p>
 * Для идентификации элементов используется <tt>==</tt>, а не
...
public class LazyArrayList<E> implements List<E>, RandomAccess, Cloneable, java.io.Serializable {
    ...
    /**
     * Получить список LazyArrayList из пула. В случае, если в пуле нет
     * свободных объектов, будет создан новый.
     *
    ...
    /**
     * Создать новый список, с начальным размером внутреннего массива
     ...
     * @return предыдущий элемент списка в заданной позиции, либо <tt>null</tt>,
     * в случае, если заданная позиция выходит за пределы размерности списка
     */
    @Override
    ...
    /**
     * Вставить указанный элемент в указанную позицию списка, при этом все
     * элементы в этой позиции сдвигаются направо
     *
    ...
    /**
     * Вставить элементы коллекции в указанную позицию списка, при этом все
     * элементы в этой позиции сдвигаются направо. Для получения элементов
     * коллекции используется метод
     * {@link Collection#toArray() Collection.toArray()}
     *
     * @param index позиция, в которую необходимо вставить элементы указанной
     * коллекции
    ...
    /**
     * Расширить внутренний массив так, чтобы он смог разместить как минимум
     * <b>newSize</b> элементов
     *
    ...
    /**
     * Удалить элемент в указаной позиции. При этом элементы не сдвигаются
     * влево, а в указанную позицию, вместо удаленного, помещается элемент с
     * конца списка
     *
    ...
    /**
     * Удалить из списка первое вхождение объекта, может быть <tt>null</tt>, при
     * проверке идиентичности используется оператор <tt>==</tt>
     *
    ...
    /**
     * Возвращает true, если объект содержится в списке, в качестве аргумента
     * может быть <tt>null</tt>, при проверке идиентичности используется
     * оператор <tt>==</tt>
     *
    ...
    /**
     * Возвращает позицию первого вхождения объекта в списке, в качестве
     * аргумента может быть <tt>null</tt>, при проверке идиентичности
     * используется оператор <tt>==</tt>, если объект не найден, возвращает -1
     *
     * @param o объект для поиска в списке
     * @return позиция, в которой находится объект в списке, либо -1, если
     * объект не найден
     */
    @Override
    ...
    /**
     * Возвращает позицию последнего вхождения объекта в списке, в качестве
     * аргумента может быть <tt>null</tt>, при проверке идиентичности
     * используется оператор <tt>==</tt>, если объект не найден, возвращает -1
     *
     * @param o объект для поиска в списке
     * @return последняя позиция, в которой находится объект в списке, либо -1,
     ...
     * @return возвращает элемент списка в заданной позиции, либо <tt>null</tt>,
     * если заданая позиция выходит за пределы размерности списка
     */
    @Override
    ...
    /**
     * Добавить все элементы коллекции в список. Для получения элементов
     * коллекции используется метод
     * {@link Collection#toArray() Collection.toArray()}
     *
     ...
     *
     * @param c коллекция, которая содержит элементы для проверки нахождения в
     * списке
    ...
    /**
     * Удаляет из списка все элементы, которые не содержатся в заданной
     * коллекции, для проверки нахождения элемента в коллекции используется
     * метод коллекции
     * {@link Collection#contains(Object) Collection.contains(Object)}
     *
     * @param c коллекция, которая содержит элементы, которые необходимо
     * оставить в списке
     * @return true, если список был изменен
     */
    @Override
    ...
    /**
     * Удаляет из списка все элементы, которые содержатся в заданной коллекции,
     * для проверки нахождения элемента в коллекции используется метод коллекции
     * {@link Collection#contains(Object) Collection.contains(Object)}
     *
