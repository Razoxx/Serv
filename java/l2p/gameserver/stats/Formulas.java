...
public class Formulas {
    ...
    /**
     * Для простых ударов patk = patk При крите простым ударом: patk = patk * (1
     * + crit_damage_rcpt) * crit_damage_mod + crit_damage_static Для blow
     * скиллов TODO Для скилловых критов, повреждения просто удваиваются, бафы
     * не влияют (кроме blow, для них выше) patk = (1 + crit_damage_rcpt) *
     * (patk + skill_power) Для обычных атак damage = patk * ss_bonus * 70 /
     * pdef
     */
    ...
    public static boolean calcSkillSuccess(Env env, EffectTemplate et, int spiritshot) {
        ...
        double mAtkMod = 1.;
        int ssMod = 0;
        if (skill.isMagic() && (et == null || et.chance() < 0)) // Этот блок только для магических скиллов, эффекты с отдельным шансом тоже пропускаются
        {
        ...
        if (lvlDependMod != 0) {
            int attackLevel = skill.getMagicLevel() > 0 ? skill.getMagicLevel() : caster.getLevel(); // Разница в уровнях
    ...
    /**
     * Возвращает множитель для атаки из значений атакующего и защитного
     * элемента.
     * <br /><br />
