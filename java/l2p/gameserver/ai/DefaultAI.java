...
public class DefaultAI extends CharacterAI {
    ...
    protected boolean maybeMoveToHome() {
        ...
        // Идём домой, только если далеко от дома
    ...
    protected void returnHome(boolean clearAggro, boolean teleport) {
        ...
        if (!GeoEngine.canMoveToCoord(actor.getX(), actor.getY(), actor.getZ(), sloc_tmp.x, sloc_tmp.y, sloc_tmp.z, actor.getGeoIndex())) // Для подстраховки
        {
    ...
    /**
     * Задержка, перед переключением в активный режим после атаки, если цель не
     * найдена (вне зоны досягаемости, убита, очищен хейт)
     *
     * @return
     */
