...
public class PlayableAI extends CharacterAI {
    ...
    protected void thinkCast(boolean arrived) {
        ...
        // фикс каста с площядок для виверн у замков :Gludio, Oren, Giran, Dion, Innadril
        if ((_skill.getSkillType() == SkillType.TAKECASTLE || _skill.getSkillType() == SkillType.TAKEFORTRESS) && Math.abs(actor.getZ() - target.getZ()) > 200) {
            actor.sendPacket(SystemMsg.CANNOT_SEE_TARGET);
            setIntention(AI_INTENTION_ACTIVE);
            actor.sendActionFailed();
            return;
