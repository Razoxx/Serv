...
import l2p.commons.crypt.CryptUtil;
...
import l2p.gameserver.stats.conditions.ConditionConfigBoolean;
import l2p.gameserver.stats.conditions.ConditionFirstEffectSuccess;
import l2p.gameserver.stats.conditions.ConditionGameTime;
import l2p.gameserver.stats.conditions.ConditionGameTime.CheckGameTime;
import l2p.gameserver.stats.conditions.ConditionHasSkill;
import l2p.gameserver.stats.conditions.ConditionLogicAnd;
import l2p.gameserver.stats.conditions.ConditionLogicNot;
import l2p.gameserver.stats.conditions.ConditionLogicOr;
import l2p.gameserver.stats.conditions.ConditionPlayerAgathion;
import l2p.gameserver.stats.conditions.ConditionPlayerBoat;
import l2p.gameserver.stats.conditions.ConditionPlayerChargesMax;
import l2p.gameserver.stats.conditions.ConditionPlayerChargesMin;
import l2p.gameserver.stats.conditions.ConditionPlayerClassId;
import l2p.gameserver.stats.conditions.ConditionPlayerCubic;
import l2p.gameserver.stats.conditions.ConditionPlayerDominionWar;
import l2p.gameserver.stats.conditions.ConditionPlayerEncumbered;
import l2p.gameserver.stats.conditions.ConditionPlayerHasBuff;
import l2p.gameserver.stats.conditions.ConditionPlayerHasBuffId;
import l2p.gameserver.stats.conditions.ConditionPlayerHasSummon;
import l2p.gameserver.stats.conditions.ConditionPlayerInstanceZone;
import l2p.gameserver.stats.conditions.ConditionPlayerIsHero;
import l2p.gameserver.stats.conditions.ConditionPlayerIsMageClass;
import l2p.gameserver.stats.conditions.ConditionPlayerMaxLevel;
import l2p.gameserver.stats.conditions.ConditionPlayerMaxPK;
import l2p.gameserver.stats.conditions.ConditionPlayerMinLevel;
import l2p.gameserver.stats.conditions.ConditionPlayerMinMaxDamage;
import l2p.gameserver.stats.conditions.ConditionPlayerOlympiad;
import l2p.gameserver.stats.conditions.ConditionPlayerPercentCp;
import l2p.gameserver.stats.conditions.ConditionPlayerPercentHp;
import l2p.gameserver.stats.conditions.ConditionPlayerPercentMp;
import l2p.gameserver.stats.conditions.ConditionPlayerRace;
import l2p.gameserver.stats.conditions.ConditionPlayerRiding;
import l2p.gameserver.stats.conditions.ConditionPlayerRiding.CheckPlayerRiding;
import l2p.gameserver.stats.conditions.ConditionPlayerState;
import l2p.gameserver.stats.conditions.ConditionPlayerState.CheckPlayerState;
import l2p.gameserver.stats.conditions.ConditionPlayerSummonSiegeGolem;
import l2p.gameserver.stats.conditions.ConditionPlayerTransformation;
import l2p.gameserver.stats.conditions.ConditionSlotItemId;
import l2p.gameserver.stats.conditions.ConditionTargetActiveSkillId;
import l2p.gameserver.stats.conditions.ConditionTargetAggro;
import l2p.gameserver.stats.conditions.ConditionTargetCastleDoor;
import l2p.gameserver.stats.conditions.ConditionTargetClan;
import l2p.gameserver.stats.conditions.ConditionTargetDirection;
import l2p.gameserver.stats.conditions.ConditionTargetForbiddenClassId;
import l2p.gameserver.stats.conditions.ConditionTargetHasBuff;
import l2p.gameserver.stats.conditions.ConditionTargetHasBuffId;
import l2p.gameserver.stats.conditions.ConditionTargetHasForbiddenSkill;
import l2p.gameserver.stats.conditions.ConditionTargetMob;
import l2p.gameserver.stats.conditions.ConditionTargetMobId;
import l2p.gameserver.stats.conditions.ConditionTargetNpcClass;
import l2p.gameserver.stats.conditions.ConditionTargetPercentCp;
import l2p.gameserver.stats.conditions.ConditionTargetPercentHp;
import l2p.gameserver.stats.conditions.ConditionTargetPercentMp;
import l2p.gameserver.stats.conditions.ConditionTargetPlayable;
import l2p.gameserver.stats.conditions.ConditionTargetPlayer;
import l2p.gameserver.stats.conditions.ConditionTargetPlayerRace;
import l2p.gameserver.stats.conditions.ConditionTargetRace;
import l2p.gameserver.stats.conditions.ConditionTargetSummon;
import l2p.gameserver.stats.conditions.ConditionUsingArmor;
import l2p.gameserver.stats.conditions.ConditionUsingBlowSkill;
import l2p.gameserver.stats.conditions.ConditionUsingItemType;
import l2p.gameserver.stats.conditions.ConditionUsingSkill;
import l2p.gameserver.stats.conditions.ConditionZoneType;
...
abstract class DocumentBase {
    ...
    protected void attachEffect(Node n, Object template) {
        ...
        if (attrs.getNamedItem("count") != null) {
            int count = parseNumber(attrs.getNamedItem("count").getNodeValue()).intValue();
            if (count == 0) {
                return;
            }
            set.set("count", count);
    ...
    protected Condition parsePlayerCondition(Node n) {
        ...
        for (int i = 0; i < attrs.getLength(); i++) {
            ...
            if ("race".equalsIgnoreCase(nodeName)) {
                ...
                cond = joinAnd(cond, new ConditionPlayerBoat(Boolean.valueOf(a.getNodeValue()).booleanValue()));
            ...
            } else if ("transformation".equalsIgnoreCase(nodeName)) {
                int transformation = parseNumber(a.getNodeValue()).intValue();
                cond = joinAnd(cond, new ConditionPlayerTransformation(transformation));
            ...
            } else if ("dominion_war".equalsIgnoreCase(nodeName)) {
                int id = parseNumber(a.getNodeValue()).intValue();
                cond = joinAnd(cond, new ConditionPlayerDominionWar(id));
                ...
                int val = parseNumber(a.getNodeValue()).intValue();
            ...
            } else if ("isMageClass".equalsIgnoreCase(nodeName)) {
                boolean val = Boolean.parseBoolean(a.getNodeValue());
                cond = joinAnd(cond, new ConditionPlayerIsMageClass(val));
    ...
    protected Condition parseConfigCondition(Node n) {
        Condition cond = null;
        NamedNodeMap attrs = n.getAttributes();
        Node name = attrs.getNamedItem("name");
        Node val = attrs.getNamedItem("val");
        if ((name != null) && (val != null)) {
            Field field = FieldUtils.getField(Config.class, name.getNodeValue());
            if (field != null) {
                if (field.getType() == Boolean.TYPE) {
                    cond = joinAnd(cond, new ConditionConfigBoolean(field, Boolean.valueOf(val.getNodeValue()).booleanValue()));
                }
            }
        }
        if (cond == null) {
            _log.error("Unrecognized <config> condition in " + this.file);
        }
        return cond;
    }
    ...
    /**
     * Разбирает параметр Value как число, приводя его к Number, либо возвращает
     * значение из таблицы если строка начинается с #
     */
