...
public class FishingSkill extends Skill {
    ...
    public boolean checkCondition(Creature activeChar, Creature target, boolean forceUse, boolean dontMove, boolean first) {
        ...
        if (zone.getParams().getInteger("distribution_id") == 4 && lure.getItemId() != 8548) {
            player.sendPacket(Msg.YOU_CANT_FISH_HERE);
            return false;
        }
        ...
        for (Zone zoneTemp : zones) {
            if (zoneTemp.getType() == ZoneType.water || (zoneTemp.getType() == ZoneType.FISHING && zoneTemp.getParams().getInteger("distribution_id") == 4)) {
                //z - уровень воды
                z = zoneTemp.getTerritory().getZmax();
                isInWater = true;
                break;
            }
        }
    ...
    public void useSkill(Creature caster, List<Creature> targets) {
        ...
        int group = l2p.gameserver.model.Fishing.getFishGroup(lureId);
