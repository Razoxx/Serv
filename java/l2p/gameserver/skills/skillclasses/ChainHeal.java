...
public class ChainHeal extends Skill {
    ...
    public List<Creature> getTargets(Creature activeChar, Creature aimingTarget, boolean forceUse) {
        ...
        for (Creature target : targets) {
            ...
            if (ConditionTargetRelation.getRelation(activeChar, target) != ConditionTargetRelation.Relation.Friend && Config.ALLOW_CHAIN_HEAL_FRIEND) {
                continue;
            }

            if (target.isCursedWeaponEquipped() && Config.ALLOW_CHAIN_HEAL_CURSED) {
                continue;
            }
            ...
            if (activeChar.getPlayer().isInPvPEvent() && target.getPlayer().isInPvPEvent() && activeChar.getPlayer().getTeam() != target.getPlayer().getTeam() && Config.ALLOW_CHAIN_HEAL_EVENT) {
                continue;
            }
            if (activeChar.getPlayer().isInLastHero() && target.getPlayer().isInLastHero() && Config.ALLOW_CHAIN_HEAL_EVENT) {
                continue;
            }
