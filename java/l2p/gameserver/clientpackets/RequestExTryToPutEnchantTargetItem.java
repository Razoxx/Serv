...
public class RequestExTryToPutEnchantTargetItem extends L2GameClientPacket {
    ...
    private static void doPutOld(Player activeChar, ItemInstance itemToEnchant, ItemInstance scroll) {
        ...
        // ольф 21706(21707)
        if (scrollId == 21707 && itemToEnchant.getItemId() != 21706) {
            activeChar.sendPacket(new ExPutEnchantTargetItemResult(0));
            activeChar.sendPacket(SystemMsg.DOES_NOT_FIT_STRENGTHENING_CONDITIONS_OF_THE_SCROLL);
            activeChar.setEnchantScroll(null);
            return;
        ...
        if (ItemFunctions.isDestructionWpnEnchantScroll(scrollId) && itemToEnchant.getEnchantLevel() >= Config.ENCHANT_MAX_DESTRUCTION_WEAPON || ItemFunctions.isDestructionArmEnchantScroll(scrollId) && itemToEnchant.getEnchantLevel() >= Config.ENCHANT_MAX_DESTRUCTION_ARMOR) {
            activeChar.sendPacket(new ExPutEnchantTargetItemResult(0));
            activeChar.sendPacket(SystemMsg.DOES_NOT_FIT_STRENGTHENING_CONDITIONS_OF_THE_SCROLL);
            activeChar.setEnchantScroll(null);
            return;
        ...
        switch (itemToEnchant.getItemId()) {
            case 13539:
                ...
                break;
            case 21580:
                if (itemToEnchant.getEnchantLevel() >= Config.MAX_ENCHANT_OLF) {
                    fail = true;
                }
                break;
            default: {
