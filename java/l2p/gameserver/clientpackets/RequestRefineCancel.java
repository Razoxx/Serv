...
public final class RequestRefineCancel extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (targetItem == null || !targetItem.isAugmented() && !(targetItem.isVisualItem() && Config.ALLOW_VISUAL_FROM_AUGMENT)) {
            activeChar.sendPacket(new ExVariationCancelResult(0), SystemMsg.AUGMENTATION_REMOVAL_CAN_ONLY_BE_DONE_ON_AN_AUGMENTED_ITEM);
            return;
        }

        if (targetItem.isVisualItem() && Config.ALLOW_VISUAL_FROM_AUGMENT) {
            boolean equipped = false;
            if (equipped = targetItem.isEquipped()) {
                activeChar.getInventory().unEquipItem(targetItem);
            }
            
            int returnitem = targetItem.getVisualItemId();

            targetItem.setVisualItemId(0);
            ...
            ItemFunctions.addItem(activeChar, returnitem, 1, equipped);
