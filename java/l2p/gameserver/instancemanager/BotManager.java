...
public class BotManager {
    ...
    public synchronized void reportBot(Player reported, Player reporter) {
        ...
        SystemMessage sm = new SystemMessage(SystemMsg.C1_WAS_REPORTED_AS_A_BOT);
        sm.addName(reported);
        reporter.sendPacket(sm);
    }

    /**
     * Will load the data from all unreaded reports (used to load reports in a
     * window for admins/GMs)
     *
     * @return a FastMap<Integer, String[]> (Integer - report id, String[] -
     * reported name, report name, date)
     */
    ...
    /**
     * Return a HashMap holding all the reports data to be viewed by any GM
     *
     * @return _unread
     */
    ...
    /**
     * Will save the punish being suffered to player in database (at player logs
     * out), to be restored next time players enter in server
     *
     * @param punished
     */
    ...
    public boolean validateBot(Player reported, Player reporter) {
        ...
        if (_reportedCount.containsKey(reporter)) {
            for (Player p : _reportedCount.get(reporter)) {
                if (reported == p) {
                    reporter.sendPacket(SystemMsg.C1_WAS_REPORTED_AS_A_BOT);
                    return false;
                }
            }
        }
    ...
    public synchronized boolean validateReport(Player reporter) {
        ...
        if (reporter.getNetConnection().getReportsPoints() == 0) {
            ...
            if (delay <= 1800000) {
                ...
                SystemMessage sm = new SystemMessage(SystemMsg.YOU_CAN_REPORT_IN_S1_MINUTES_S2_REPORT_POINTS_REMAIN_IN_ACCOUNT);
                sm.addNumber(left);
                sm.addNumber(reporter.getNetConnection().getReportsPoints());
                reporter.sendPacket(sm);
                return false;
            } else {
