...
public class CastleManorManager {
    ...
    private List<SeedProduction> getNewSeedsList(int castleId) {
        List<SeedData> allSeeds = Manor.getInstance().getSeedsForCastle(castleId);
        List<SeedProduction> seeds = new ArrayList<SeedProduction>(allSeeds.size());
        for (SeedData sd : allSeeds) {
    ...
    private List<CropProcure> getNewCropsList(int castleId) {
        List<SeedData> cropsIds = Manor.getInstance().getCropsForCastle(castleId);
        List<CropProcure> crops = new ArrayList<CropProcure>(cropsIds.size());
        for (SeedData sd : cropsIds) {
            crops.add(new CropProcure(sd.getCrop()));
        }
        return crops;
    }
