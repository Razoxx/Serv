...
public class SMSWayToPay {
    ...
    private static final Logger _log = LoggerFactory.getLogger(SMSWayToPay.class.getName());
    ...
    private static final String SELECT_RPOFIT_SMS_DATA = "SELECT profit FROM character_sms_donate WHERE wText=? AND has_reward=0";
    ...
    public void parse() {
        ...
        for (String arrayedData : arrayedDatas) {
            ...
            if (formedStr.length > 1) {
                ...
                /*_log.info("================================");
                 _log.info("id =" + dc.id);
                 _log.info("service_id = " + dc.service_id);
                 _log.info("status = " + dc.status);
                 _log.info("time = " + dc.time);
                 _log.info("curr_id = " + dc.curr_id);
                 _log.info("sum = " + dc.sum);
                 _log.info("profit = " + dc.profit);
                 _log.info("email = " + dc.email);
                 _log.info("client_id = " + dc.client_id);
                 _log.info("cparams = " + dc.client_params);
                 _log.info("--------------------------------");
                 _log.info("cparams.get(\"wNumber\") = " + Integer.parseInt(outputCollection.get("wNumber")));
                 _log.info("cparams.get(\"wPhone\") = " + outputCollection.get("wPhone"));
                 _log.info("cparams.get(\"wText\") = " + outputCollection.get("wText"));
                 _log.info("cparams.get(\"wCost\") = " + Float.parseFloat(outputCollection.get("wCost")));
                 _log.info("cparams.get(\"wProfit\") = " + Float.parseFloat(outputCollection.get("wProfit")));
                 _log.info("cparams.get(\"wCountry\") = " + outputCollection.get("wCountry"));*/
            }
        }
    }
    ...
    public void checkAndSave(int id, int service_id, int status, long time, int curr_id, float sum, float profit, String email, int client_id, int wNumber, String wPhone, String wText, float wCost, float wProfit, String wCountry) {
        ...
        try {
            ...
            _log.info("Error while saving SMS data.", e);
        } finally {
        ...
        try {
            ...
            for (Player player : GameObjectsStorage.getAllPlayers()) {
                ...
                while (rsetProfit.next()) {
                    profit += rsetProfit.getFloat("profit");
                }
                ...
                if (profit > 0) {
                    ...
                    if (player.getVar("lang@").equalsIgnoreCase("ru")) {
                        player.sendMessage("Администрация сервера " + Config.SMS_PAYMENT_SERVER_ADDRESS + " благодарит вас за SMS");
                    } else {
