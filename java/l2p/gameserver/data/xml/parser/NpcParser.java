...
public final class NpcParser extends AbstractDirParser<NpcHolder> {
    ...
    protected void readData(org.dom4j.Element rootElement) throws Exception {
        for (Iterator<org.dom4j.Element> npcIterator = rootElement.elementIterator(); npcIterator.hasNext();) {
            ...
            for (Iterator<org.dom4j.Element> secondIterator = npcElement.elementIterator(); secondIterator.hasNext();) {
                ...
                if (nodeName.equalsIgnoreCase("faction")) {
                    ...
                    for (Iterator<org.dom4j.Element> nextIterator = secondElement.elementIterator(); nextIterator.hasNext();) {
                        ...
                        int min = Integer.parseInt(nextElement.attributeValue("min"));
                        int max = Integer.parseInt(nextElement.attributeValue("max"));

                        template.addMinion(new MinionData(id, min, max));
                    }
            ...
            /*        Map<Integer, Integer> _skillList = NpcSkillDAO.getInstance().select(npcId);
             if (_skillList != null && !_skillList.isEmpty()) {
             for (Map.Entry<Integer, Integer> e : _skillList.entrySet()) {
             ...
             Skill skill = SkillTable.getInstance().getInfo(id, level);
             if (skill == null) {
             _log.info("NOTDONE SKILL skilId=" + id + " lvl=" + level);
             continue;
             }
             // Для определения расы используется скилл 4416
             if (id == 4416) {
    ...
    private RewardData parseReward(org.dom4j.Element rewardElement, int id) {
        ...
        double chance_adena = chance * Config.RATE_CHANCE_DROP_ADENA;
        ...
        if (chance_adena > 1000000) {
            chance_adena = 1000000;
        }
        ...
        if (id == 1) {
            ...
            } else if (data.getItem().isAdena()) {
                data.setChance(chance_adena);
            } else {
        ...
        if (data.getItem().isStonePrint()) {
            data.setMinDrop(min * (int) Config.RATE_DROP_STONEPRINT);
            data.setMaxDrop(max * (int) Config.RATE_DROP_STONEPRINT);
        }
