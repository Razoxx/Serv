    ...
    public boolean useAdminCommand(Enum comm, String[] wordList, String fullString, Player activeChar) {
        ...
        switch (command) {
            ...
            case admin_unpara:
                ...
                if (((Creature) target).isParalyzed()) {
                    ((Creature) target).stopParalyzed();
                    activeChar.sendMessage("Target stop paralyzed.");
                } else{
                    ((Creature) target).startParalyzed();
                    ((Creature) target).abortAttack(true, true);
                    ((Creature) target).abortCast(true, true);
                    if (((Creature) target).isMoving) {
                        ((Creature) target).stopMove();
                    }
                    activeChar.sendMessage("Target paralyzed.");
                }
                break;
        }
