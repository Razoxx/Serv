...
public class GameObjectsStorage {
    ...
    /**
     * копия списка игроков из хранилища, пригодна для последующих манипуляций
     * над ней для перебора в основном лучше использовать
    ...
    /**
     * использовать только для перебора типа for(L2Player player :
     * getAllPlayersForIterate()) ...
     */
    ...
    /**
     * кладет объект в хранилище и возвращает уникальный индентификатор по
     * которому его можно будет найти в хранилище
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * генерирует уникальный индентификатор по которому будет ясно что объект
     * вне хранилища но можно будет получить objectId
     */
