...
public abstract class Skill extends StatTemplate implements Cloneable {
     ...
     *
     * @param effector персонаж, со стороны которого идет действие скилла,
     * кастующий
     * @param effected персонаж, на которого действует скилл
     * @param calcChance если true, то расчитывать шанс наложения эффекта
     * @param applyOnCaster если true, накладывать только эффекты
     * предназанченные для кастующего
     * @param timeConst изменить время действия эффектов до данной константы (в
     * миллисекундах)
     * @param timeMult изменить время действия эффектов с учетом данного
     * множителя
                ...
                for (EffectTemplate et : getEffectTemplates()) {
                    ...
                    for (int i = 0; i < targets.length; i++) {
                        ...
                        if (calcChance && !et._applyOnCaster) {
                            ...
                            } else if (!calcBase) // еще не считалось
                            {
                                calcBase = true;
    ...
    /**
	 * @return true если эффекты временно, как хербы, а не постоянно, перекрывают уже имеющиеся
	 */
