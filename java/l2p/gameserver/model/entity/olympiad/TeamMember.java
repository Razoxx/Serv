...
public class TeamMember {
    ...
    public boolean checkPlayer() {
        ...
        if (player._event != null) {
            player.sendMessage(player.isLangRus() ? "Вы уже зарегестрированы на другом эвенте." : "You have already registered at another opening event.");
            return false;
        }
        return true;
    }
    ...
    public void preparePlayer() {
        ...
        // Удаляем баффы и чужие кубики
        if (Config.OLYMPIAD_BUFF_DELETED) {
            for (Effect e : player.getEffectList().getAllEffects()) {
                if (e.getEffectType() != EffectType.Cubic || player.getSkillLevel(e.getSkill().getId()) <= 0) {
                    e.exit();
                }
