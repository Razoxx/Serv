...
public class Coliseum {
    ...
    private void load() {
        ...
        if (_event_cycle.intValue() == 2147483647) {
            ...
            Integer localInteger1 = _event_cycle;
            localInteger2 = Coliseum._event_cycle = Integer.valueOf(_event_cycle.intValue() + 1);
        }
    }
            ...
            if (_winner == TeamType.NONE) {
                ...
                if (_party1.getPartyLeader().getTeam() == _winner) {
                    if (!getPreviusWinners().equals(_party1)) {
                        ...
                        setWinCount(1);
                        for (Player member : _party1.getPartyMembers()) {
                ...
                if (_party2.getPartyLeader().getTeam() == _winner) {
                    if (!getPreviusWinners().equals(_party2)) {
                        ...
                        setWinCount(1);
                        for (Player member : _party2.getPartyMembers()) {
                            member.setFame(member.getFame() + 80, "Coliseum");
                        }
                    } else {
                        incWinCount();
