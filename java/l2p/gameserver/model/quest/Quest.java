...
public class Quest {
    ...
    /**
     * Этот метод для регистрации квестовых вещей, которые будут удалены при
     * прекращении квеста, независимо от того, был он закончен или прерван.
    ...
    /**
     * Добавляет нпц масив для слушателя при их убийстве, и обновлении пакетом
     * {@link l2p.gameserver.network.l2.s2c.ExQuestNpcLogList}
     *
     * @param cond
     * @param varName
     * @param killIds
    ...
    /**
     * Возвращает название квеста (Берется с npcstring-*.dat) state 1 = "" state
     * 2 = "In Progress" state 3 = "Done"
     */
    ...
    protected void showHtmlFile(Player player, String fileName, boolean showQuestInfo, Object... arg) {
        ...
        if (showQuestInfo) {
            npcReply.setQuestId(getQuestIntId());
        }
     ...
     * <LI><U>"res" starts with tag "html" :</U> the message hold in "res" is
