...
public final class QuestState {
     ...
     * <br><br>
     * Метод учитывает рейты! 3-ий параметр true/false показывает является ли
     * квест на профессию и рейты учитываются в завимисомти от параметра
    ...
    public void addExpAndSp(long exp, long sp) {
        ...
        if (exp > 0) {
            player.addExpAndSp((long) (exp * getRateQuestsReward() * getRateValue(Config.EXP)), 0);
        }
        if (sp > 0) {
            player.addExpAndSp(0, (long) (sp * getRateQuestsReward() * getRateValue(Config.SP)));
        }
    }
    ...
    public QuestState exitCurrentQuest(boolean repeatable) {
        ...
        if (repeatable) {
            ...
            CharacterQuestDAO.getInstance().delete(getPlayer().getObjectId(), _quest._name);
            _vars.clear();
    ...
    /**
     * Добавить предмет игроку By default if item is adena rates 'll be applyed,
     * else no
     *
     * @param itemId
     * @param count
    ...
    /**
     * Этот метод рассчитывает количество дропнутых вещей в зависимости от
     * рейтов.
     * <br><br>
     * Следует учесть, что контроль за верхним пределом вещей в квестах, в
     * которых нужно набить определенное количество предметов не осуществляется.
     * <br><br>
    ...
    public double getRateQuestsDrop() {
        Player player = getPlayer();
        double Bonus = player == null ? 1. : (player.hasBonus() ? PremiumConfig.getPremConfigId(player.getBonus().getBonusId()).RATE_QUEST_DROP : 1);
        return Config.RATE_QUESTS_DROP * Bonus * getRateValue(Config.DROP);
    }

    public double getRateQuestsReward() {
        Player player = getPlayer();
        double Bonus = player == null ? 1. : (player.hasBonus() ? PremiumConfig.getPremConfigId(player.getBonus().getBonusId()).RATE_QUEST_REWARD : 1);
        return Config.RATE_QUESTS_REWARD * Bonus * getRateValue(Config.REWARD);
    }

    /**
     * Этот метод рассчитывает количество дропнутых вещей в зависимости от
     * рейтов и дает их, проверяет максимум, а так же проигрывает звук получения
     * вещи.
     * <br><br>
     * Ни один из передаваемых параметров не должен быть равен 0
     *
    ...
    /**
     * Этот метод рассчитывает количество дропнутых вещей в зависимости от
     * рейтов и дает их, а так же проигрывает звук получения вещи.
     * <br><br>
     * Следует учесть, что контроль за верхним пределом вещей в квестах, в
     * которых нужно набить определенное количество предметов не осуществляется.
     * <br><br>
     ...
     * <p/>
     * Устанавливает переменную и сохраняет в базу, если установлен флаг. Если
     * получен cond обновляет список квестов игрока (только с флагом).
     *
     * @param var : String pointing out the name of the variable for quest
     * @param val : String pointing out the value of the variable for quest
     * @param store : Сохраняет в базу и если var это cond обновляет список
     * квестов игрока.
     ...
     * quest.<BR><BR>
     * <U><I>Concept : </I></U>
     * Remove the variable of quest represented by "var" from the class variable
     * FastMap "vars" and from the database.
     *
    ...
    public String unset(String var) {
        ...
        if (old != null) {
            CharacterQuestDAO.getInstance().delete(getPlayer().getObjectId(), _quest._name, var);
        }
        return old;
    }
    ...
    public String setCond(int newCond, boolean store) {
        ...
        String result = set(VAR_COND, sVal, false);
        if (store) {
    ...
    /**
     * Устанавлевает время, когда квест будет доступен персонажу. Метод
     * используется для квестов, которые проходятся один раз в день.
     */
    ...
    /**
     * Проверяет, наступило ли время для выполнения квеста. Метод используется
     * для квестов, которые проходятся один раз в день.
     *
     * @return boolean
     */
    ...
    public double getRateValue(int rate) {
        double[] rates = (double[]) Config.QUEST_RATES.get(getQuest().getClass().getSimpleName());
        rates = rates == null ? Config.EMPTY_RATES : rates;
        return rates[rate];
    }
}
