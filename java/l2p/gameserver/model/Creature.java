...
import l2p.gameserver.model.GameObjectTasks.AltMagicUseTask;
import l2p.gameserver.model.GameObjectTasks.CastEndTimeTask;
import l2p.gameserver.model.GameObjectTasks.HitTask;
import l2p.gameserver.model.GameObjectTasks.MagicLaunchedTask;
import l2p.gameserver.model.GameObjectTasks.MagicUseTask;
...
public abstract class Creature extends GameObject {
    ...
    /**
     * Список игроков, которым необходимо отсылать информацию об изменении
     * состояния персонажа
     */
    ...
    public double absorbToSummon(Creature attacker, double damage) {
        ...
        if (transferToSummonDam > 0) {
            ...
            if (summon == null || summon.getCurrentHp() < transferDamage || getEffectList().getEffectByType(EffectType.Petrification) != null) {
                getEffectList().stopEffects(EffectType.AbsorbDamageToSummon);
            } else if (summon.isSummon() && summon.isInRangeZ(this, 1200) || !summon.isDead()) {
                damage -= transferDamage;
    ...
    public void doCounterAttack(Skill skill, Creature attacker, boolean blow) {
        ...
        int numberOfAttacks = skill.getNumberOfCounterAttacks();
    ...
    protected void onDeath(Creature killer) {
        ...
        if (isBlessedByNoblesse() || isSalvation()) {
            ...
            for (Effect e : getEffectList().getAllEffects()) // Noblesse Blessing Buff/debuff effects are retained after
            ...
            for (Effect e : getEffectList().getAllEffects()) // Некоторые эффекты сохраняются при смерти
            {
     ...
     */
    // Умножаем на 10, так как в парсере этого не сделали)
    public int getCriticalHit(Creature target, Skill skill) {
        if (this instanceof Player) {
            return (int) calcStat(Stats.CRITICAL_BASE, _template.baseCritRate, target, skill);
        }

        return (int) calcStat(Stats.CRITICAL_BASE, _template.baseCritRate * 10, target, skill);
    }
    ...
    public int getPAtkSpd(boolean applyLimit) {
        int result = (int) calcStat(Stats.POWER_ATTACK_SPEED, _template.basePAtkSpd, null, null);
    ...
    /**
     * Возвращает координаты поверхности воды, если мы находимся в ней, или над
     * ней.
     */
    ...
    public void onHitTimer(Creature target, int damage, boolean crit, boolean miss, boolean soulshot, boolean shld, boolean unchargeSS) {
        ...
        if (miss && (Math.abs(this.getZ() - target.getZ()) < 200)) {
    ...
    public void reduceCurrentHp(double damage, Creature attacker, Skill skill, boolean awake, boolean standUp, boolean directHp, boolean canReflect, boolean transferDamage, boolean isDot, boolean sendMessage) {
        ...
        if (isPlayer() && attacker.isPlayer() && getPlayer().getGameEvent() != null && attacker.getPlayer().getGameEvent() != null
                && attacker.getPlayer().getGameEvent() == getPlayer().getGameEvent()
                && getPlayer().getPvPTeam() == attacker.getPlayer().getPvPTeam()) {
            return;
        }
    ...
    /**
     * Запускаем задачу анимации боевой позы. Если задача уже запущена,
     * увеличиваем время, которое персонаж будет в боевой позе на 15с
     */
    ...
    public void stopAbnormalEffect(AbnormalEffect ae) {
        _abnormalEffects[ae.getType().ordinal()] &= (ae.getMask() ^ 0xFFFFFFFF);
        sendChanges();
    }
    ...
    public void setTeam(TeamType t) {
        _checksForTeam = t != TeamType.NONE;
        _team = t;
        sendChanges();
    }
    ...
    /**
     * Выставить предельные значения HP/MP/CP и запустить регенерацию, если в
