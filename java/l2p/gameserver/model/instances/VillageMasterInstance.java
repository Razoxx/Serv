...
public final class VillageMasterInstance extends NpcInstance {
    ...
    public void onBypassFeedback(Player player, String command) {
        ...
        if (command.startsWith("create_ally") && command.length() > 12) {
            ...
            // Саб класс нельзя получить или поменять, пока игрок находится в дуели
            if (player.isInDuel()) {
                return;
            }
    ...
    private void dissolveClan(Player player) {
        ...
        clan.setDissolvedClanTime(System.currentTimeMillis() + Config.ALT_CLAN_RESTORE_DAY * 86400000L); //24*60*60*1000 = 86400000
        clan.updateClanInDB();
        ...
        switch (clan.getLevel()) {
            ...
            case 10:
                ...
                if (clan.getReputationScore() >= Config.CLAN_LEVEL_11_COST && clan.getAllSize() >= Config.CLAN_LEVEL_11_REQUIREMEN) {
                    ...
                    if (castle == null) {
                        player.sendMessage(player.isLangRus() ? "Ваш клан не имеет ни 1-го замка!!!" : "Your clan has no 1st of the castle!");
    ...
    private Set<PlayerClass> getAvailableSubClasses(Player player, boolean isNew) {
         ...
         * Spellhowler
         *
         * Kamael могут брать только сабы Kamael Другие классы не могут брать
         * сабы Kamael
         *
         */
