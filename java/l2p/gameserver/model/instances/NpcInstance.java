...
public class NpcInstance extends Creature {
    ...
    public NpcInstance(int objectId, NpcTemplate template) {
        ...
        setEnchantItemLevel(getTemplate().enchant);
        int _weaponEnchant = getTemplate().enchant;
        _weaponEnchant = _weaponEnchant == 0 && Rnd.chance(Config.MONSTER_WEAPON_ENCHANT_CHANCE) ? Rnd.get(Config.MONSTER_WEAPON_ENCHANT_MIN, Config.MONSTER_WEAPON_ENCHANT_MAX) : _weaponEnchant;
        setEnchantItemLevel(_weaponEnchant);
    ...
    public int getEnchantItemLevel() {
        return _currentEnchantLevel;
    }
    ...
    public void setEnchantItemLevel(int newEnchant) {
        _currentEnchantLevel = newEnchant;
    }
    ...
    public boolean isAggressive() {
        return getTemplate().getAgressive() && getAggroRange() > 0;
    }
    ...
    public int getAggroRange() {
        ...
        return (getTemplate().getAgressive() && getTemplate().aggroRange == 1000) ? getTemplate().aggroRange / 2 : getTemplate().aggroRange;
    }

    /**
     * Устанавливает данному npc новый aggroRange. Если установленый aggroRange
     * < 0, то будет братся аггрорейндж с темплейта. @param aggroRange новый
     * agrroRange
     */
    ...
    public int getMDef(Creature target, Skill skill) {
        return (int) (super.getMDef(target, null) * Config.ALT_NPC_MDEF_MODIFIER);
    }
    ...
    public static boolean canBypassCheck(Player player, NpcInstance npc) {
        if (npc == null || player.isDead() || (!Config.ALLOW_TALK_WHILE_SITTING && player.isSitting()) || !npc.isInRange(player, INTERACTION_DISTANCE) || ((player.isInDuel() || player.InIsDuel()) && !Config.ALLOW_HTML_IN_DUEL)) {
            player.sendActionFailed();
            return false;
        }
        return true;
    ...
    public void onBypassFeedback(Player player, String command) {
        ...
        try {
            if (command.equalsIgnoreCase("TerritoryStatus")) {
                ...
                for (SkillLearn skill : skills) {
                    if (player.removeSkill(skill.getId(), true, false) != null) {
            ...
            for (TeleportLocation tl : list) {
                if (tl.getItem().getItemId() == ItemTemplate.ITEM_ID_ADENA) {
                    ...
                    if (Config.ALLOW_MULTILANG_GATEKEEPER) {
                        if (player.getVar("tplang@").equalsIgnoreCase("ru")) {
    ...
    public void showChatWindow(Player player, int val, Object... replace) {
        ...
        HtmlMessage packet = new HtmlMessage(player, this, filename, val);
    ...
    public void showChatWindow(Player player, String filename, Object... replace) {
        HtmlMessage packet = new HtmlMessage(player, this, filename, 0);
        if (replace.length % 2 == 0) {
            for (int i = 0; i < replace.length; i += 2) {
                packet.replace(String.valueOf(replace[i]), String.valueOf(replace[i + 1]));
            }
