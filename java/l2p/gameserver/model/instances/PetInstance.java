...
public class PetInstance extends Summon {
    ...
    class FeedTask extends RunnableImpl {
        ...
        public void runImpl() throws Exception {
            ...
            if (isUncontrollable()) {
                // Owl Monk удаляет пк в тот момент пока не поддаётся контролю.
    ...
    public void tryFeedItem(ItemInstance item) {
        ...
        if (food != null) {
            ...
            /*
             if (useItem(food, false)) {
             _log.info("Use Food " + food.getItemId());
    ...
    public void setNpcState(int val, boolean send) {
        if (_npcState != val) {
            if (send) {
                ...
                getPlayer().broadcastPacket(new L2GameServerPacket[]{packet});
            }
            _npcState = val;
        }
    }
    ...
    public int getPDef(Creature target) {
        ...
        return (int) calcStat(Stats.POWER_DEFENCE, _data.getPDef(_level) * mod, target, null);
    }
