...
public class World {
     ...
     */
    public static void showObjectsToPlayer(Player player, boolean isObserver) {
        WorldRegion currentRegion = isObserver ? player.getObservePoint().getCurrentRegion() : player.getCurrentRegion();
        if (currentRegion == null) {
            return;
        }

        int oid = player.getObjectId();
        int rid = isObserver ? player.getObservePoint().getReflectionId() : player.getReflectionId();
     ...
     */
    public static void removeObjectsFromPlayer(Player player, boolean isObserver) {
        WorldRegion currentRegion = isObserver ? player.getObservePoint().getCurrentRegion() : player.getCurrentRegion();
        if (currentRegion == null) {
            return;
        }
     ...
     */
    public static void removeObjectFromPlayers(Player player, boolean isObserver) {
        WorldRegion currentRegion = isObserver ? player.getObservePoint().getCurrentRegion() : player.getCurrentRegion();
        if (currentRegion == null) {
            return;
        }
